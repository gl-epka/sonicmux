...
This document is the pre-code CLI contract. Exact wrapping and option ordering
can change when generated by `clap`, but names, meanings, conflicts, output
channels, and exit codes require a design change after M0 approval.
...
disabled when stdout is not a terminal or either JSON mode is active.

Global configuration environment variables use the `SONICMUX_` prefix, for
example `SONICMUX_FFMPEG_PATH`, `SONICMUX_PROFILE`, and `SONICMUX_JOBS`.
...
Options:
      ...
      --show-raw             Include retained raw backend fields in JSON output
  -h, --help                 Print help
```
...
Output and safety:
  -o, --output <PATH>            Output path; requires exactly one input file
      ...
      --in-place                 Replace each source only after validation and keep .bak
      --backup-suffix <SUFFIX>   Backup suffix for --in-place [default: .bak]
      --overwrite                Replace a non-matching existing output
      --dry-run                  Probe and print plans without writing files
  -y, --yes                      Skip confirmation for already requested risky actions

Scheduling:
  -j, --jobs <N>                 Maximum concurrent files
      --storage-profile <TYPE>   Concurrency preset [default: balanced]
                                 [values: hdd, balanced, nvme]

Other:
      --continue-on-error        Finish the batch after a file fails [default: true]
      --fail-fast                Stop admitting work after the first failure
  -h, --help                     Print help
```
...
`--output`, `--output-dir`, and `--in-place` are mutually exclusive where their
meaning would overlap. `--overwrite` requires confirmation on a TTY unless
`--yes` is supplied; on a non-TTY it requires `--yes`. `--yes` alone never turns
on overwrite or in-place behavior.
...
`config init` refuses to replace an existing file unless `--overwrite` is
explicit. `config show --sources` annotates each effective value with `cli`,
